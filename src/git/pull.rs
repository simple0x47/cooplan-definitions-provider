/*
 * A big hug to the libgit2 and git2-rs contributors from simple0x47!
 *
...
fn do_fetch<'a>(
    ...
    match remote.name() {
        Some(name) => {
            println!("Fetching {} for repo", name);
        }
        None => {
            println!("Remote name is none");
        }
    }
...
fn fast_forward(
    ...
    repo.checkout_head(Some(
        git2::build::CheckoutBuilder::default()
            // Discard local changes, since there are not supposed to be any for the purposes
            // of this definition provider.
            .force(),
    ))?;
    Ok(())
}
